...
const CAPITALS_BY_STATE_NAME = _.reduce(
  ...
  (obj, capital, stateName) => {
    return { ...obj, [stateName.toLowerCase()]: capital };
  },
  {}
);
...
const launchRequestHandler = {
  ...
  handle(input) {
    ...
    return input.responseBuilder.speak(output).reprompt(reprompt).getResponse();
  },
};
...
const helpIntentHandler = {
  ...
  handle(input) {
    const output =
      "I'm an Alexa skill that can tell you the capital of any US state. Isn't that neat? Go ahead and ask me now.";
    const reprompt =
      "Ask me for the capital of a US state and I'll give you the answer.";
...
const noOrCancelOrStopIntentHandler = {
  ...
  handle(input) {
    ...
    return input.responseBuilder.speak(output).getResponse();
  },
};

// FallbackIntent triggers when a customer says something that doesn’t map to
// any intents in your skill.
...
// SessionEndedRequest notifies that a session was ended and will be triggered
// when a currently open session is closed for one of the following reasons:
//
// 1. The user says "exit" or "quit".
// 2. The user does not respond or says something that does not match an intent
//    defined in your voice model.
// 3. An error occurs
//
...
// This handler is called when a fatal exception occurs.
const errorHandler = {
  canHandle() {
    return true;
  },
  handle(input, error) {
    const output =
      "Oh dear... something seems to have gone wrong. There's not much more I can do now. Bye!";
